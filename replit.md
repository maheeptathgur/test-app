...
- July 7, 2025. Added comprehensive branding section to workspace settings - implemented three-color customization system allowing users to select primary color (buttons/accents), content area background, and sidebar background with automatic text contrast adjustment, percentage shading generation for all UI elements, live color preview with sample interface, and reset to brand defaults functionality while preserving original Knolli theme as default
```
