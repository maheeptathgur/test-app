...
- July 4, 2025. Enhanced conversation cards with full-width titles and pinning functionality - conversation titles now extend to the card edge for maximum space utilization, added pin/unpin functionality with Pin icon that toggles conversation position to top of list, pinned conversations show brand-colored pin icon and have always-visible action buttons, unpinned conversations show buttons only on hover, implemented sorting to show pinned conversations first
```
